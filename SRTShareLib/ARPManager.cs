...
namespace SRTShareLib
{
    public class ARPManager
    {
        ...
        {
            sameSubnet = new IpV4Address(to_ip).IsInSubnet(NetworkManager.Mask, NetworkManager.DefaultGateway);
            // operate by if it not in the same subnet (get gateway mac, send to gatway.. etc..)

            if (!sameSubnet)
                to_ip = NetworkManager.DefaultGateway;  // not in the same gateway -> get getway mac (via arp) and send the packet to him.
            ...
            new EthernetLayer
            {
                Source = new MacAddress(NetworkManager.MacAddress),
            ...
            new ArpLayer
            {
                ...
                SenderHardwareAddress = new MacAddress(NetworkManager.MacAddress).ToBytes().AsReadOnly(),
                SenderProtocolAddress = new IpV4Address(NetworkManager.LocalIp).ToBytes().AsReadOnly(),
