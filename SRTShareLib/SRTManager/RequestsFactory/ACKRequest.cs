...
namespace SRTShareLib.SRTManager.RequestsFactory
{
    public class ACKRequest : UdpPacket
    {
        ...
        public Packet NotifyReceived(uint ack_sequence_number, uint dest_socket_id = 0, bool videoStage = false, PeerEncryptionData peerEncryption = default)
        {
            GetPayloadLayer() = OSIManager.BuildPLayer(new ACK(dest_socket_id, ack_sequence_number).GetByted(), videoStage, peerEncryption);
            return BuildPacket();
        }
    }
}