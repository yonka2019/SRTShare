...
namespace SRTShareLib.SRTManager.RequestsFactory
{
    public class HandshakeRequest : UdpPacket
    {
        ...
        public Packet Induction(uint init_psn, IpV4Address p_ip, bool clientSide, uint source_socket_id, uint dest_socket_id, ushort encryption_type, byte[] encryption_public_key)
        {
            ...
            if (clientSide)
            {
                ...
                F_Handshake = new Handshake(version: 4, encryption_type, encryption_public_key, init_psn, type: (uint)Handshake.HandshakeType.INDUCTION, source_socket_id, dest_socket_id, p_ip);
            }
            else
            {
                // LISTENER -> CALLER (first message RESPONSE) [SERVER -> CLIENT]
                F_Handshake = new Handshake(version: 5, encryption_type, encryption_public_key, init_psn, type: (uint)Handshake.HandshakeType.INDUCTION, source_socket_id, dest_socket_id, p_ip);
            }
        ...
        public Packet Conclusion(uint init_psn, IpV4Address p_ip, bool clientSide, uint source_socket_id, uint dest_socket_id, ushort encryption_type, byte[] encryption_public_key)
        {
            Handshake F_Handshake;
            ...
            if (clientSide)
            {
                ...
                F_Handshake = new Handshake(version: 5, encryption_type, encryption_public_key, init_psn, type: (uint)Handshake.HandshakeType.CONCLUSION, source_socket_id, dest_socket_id, p_ip);
            }
            else
            {
