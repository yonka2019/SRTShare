...
namespace SRTShareLib.SRTManager.RequestsFactory
{
    public class HandshakeRequest : UdpPacket
    {
        ...
        public Packet Induction(uint init_psn, IpV4Address p_ip, bool clientSide, uint source_socket_id, uint dest_socket_id, ushort encryption_type, byte[] encryption_public_key, bool retransmission_mode)
        {
            ...
            if (clientSide)
            {
                ...
                F_Handshake = new Handshake(version: 4, encryption_type, encryption_public_key, retransmission_mode, init_psn, type: (uint)Handshake.HandshakeType.INDUCTION, source_socket_id, dest_socket_id, p_ip);
            }
            else
            {
        ...
        public Packet Conclusion(uint init_psn, IpV4Address p_ip, bool clientSide, uint source_socket_id, uint dest_socket_id, ushort encryption_type, byte[] encryption_public_key, bool retransmission_mode)
        {
            Handshake F_Handshake;
            ...
            if (clientSide)
            {
                ...
                F_Handshake = new Handshake(version: 5, encryption_type, encryption_public_key, retransmission_mode, init_psn, type: (uint)Handshake.HandshakeType.CONCLUSION, source_socket_id, dest_socket_id, p_ip);
            }
            else
            {
