                ...
                if (SRTControl.SRTHeader.IsControl(payload)) // check if control
                {
                    ...
                    if (SRTControl.Handshake.IsHandshake(payload)) // check if handshake
                    {
                        ...
                        if (handshake_request.TYPE == (uint)SRTControl.Handshake.HandshakeType.INDUCTION) // client -> server (induction)
                        {
                            ...
                            Packet handshake_packet = handshake_response.Induction(cookie, init_psn: 0, p_ip: new byte[16], clientSide: false, SERVER_SOCKET_ID, handshake_request.SOCKET_ID); // ***need to change peer id***
                        ...
                        else if (handshake_request.TYPE == (uint)SRTControl.Handshake.HandshakeType.CONCLUSION) // client -> server (conclusion)
                        {
                            ...
                            Packet handshake_packet = handshake_response.Conclusion(init_psn: 0, p_ip: new byte[16], clientSide: false, SERVER_SOCKET_ID, handshake_request.SOCKET_ID); // ***need to change peer id***
                            PacketManager.SendPacket(handshake_packet);
